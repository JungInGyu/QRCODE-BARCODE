    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()) {
            case R.id.item1:
                type = 1;
                if (color) {
                    btn.setText("Generate QR CODE (Color)");
                } else {
                    btn.setText("Generate QR CODE ");
                }
            ...
            case R.id.item2:
                type = 2;
                if (color) {
                    btn.setText("Generate Bar CODE (Color)");
                } else {
                    btn.setText("Generate Bar CODE ");
                }
                item.setChecked(true);
                break;
            case R.id.item_color:
                if (!color) {
                    item.setChecked(true);
                    btn.setText(btn.getText() + "(Color)");
                } else {
                    item.setChecked(false);
                    btn.setText(btn.getText().subSequence(0, 17));
                }
                color = !color;
        }
        return true;
            ...
            public void onClick(View view) {
                ...
                if (text.equals("")) {
                    ...
                    if (type == 1) {
                        ...
                        try {
                            bitMatrix = qrCodeWriter.encode(text, BarcodeFormat.QR_CODE, 1000, 500);
